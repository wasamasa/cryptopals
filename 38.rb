...
#
# Note that in this protocol, the server's "B" parameter doesn't
# depend on the password (it's just a Diffie Hellman public key).
#
...
def login_client(email, password)
  ...
  _A = modexp(g, a, N)
  send(S, "#{email} #{_A}")
  salt, _B, u = recv(C).split(' ').map(&:to_i)
  x = sha256_hexdigest("#{salt}#{password}".bytes).to_i(16)
...
def crack_hmac(hmac, params)
  ...
  _S = modexp(_A * modexp(v, u, N), b, N)
  _K = sha256_hexdigest(_S.to_s.bytes)
  assert(sha256_hmac(_K.bytes, salt.to_s) == hmac)
  password
end

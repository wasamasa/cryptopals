...
#
# ... decrypts to something approximating English in CTR mode, which
# is an AES block cipher mode that turns AES into a stream cipher,
...
# into stream ciphers, because most of what we want to encrypt is
# better described as a stream than as a sequence of blocks. Daniel
# Bernstein once quipped to Phil Rogaway that good cryptosystems don't
# need the "decrypt" transforms. Constructions like CTR are what he
# was talking about.
...
puts str(aes_ctr_decrypt(input, nonce, key))
assert(aes_ctr_encrypt(aes_ctr_decrypt(input, nonce, key), nonce, key) == input)
